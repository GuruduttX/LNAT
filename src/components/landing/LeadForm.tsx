...
const courses = ["LNAT", "NLSAT-LLB", "CUET PG-LLB", "MH CET", "Other"];
...
const LeadForm = ({ onSuccess, className = "", title = "Get Free Demo", compact = false }: LeadFormProps) => {
  ...
  return (
    <div className={`${compact ? "" : "bg-white p-6 border border-gray-100 shadow-sm"} ${className}`} id="hero-form">
      {!compact && (
        <>
          <h3 className="font-heading text-gray-900 mb-1" style={{ fontSize: "1.25rem", fontWeight: 900 }}>
            {title}
          </h3>
          <p className="font-body text-sm text-gray-500 mb-5">Book a free LNAT session — no commitment, no spam</p>
        </>
      )}
